...
static long gc4c33_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		hdr = (struct rkmodule_hdr_cfg *)arg;
		if (hdr->hdr_mode == gc4c33->cur_mode->hdr_mode)
			return 0;
		w = gc4c33->cur_mode->width;
		h = gc4c33->cur_mode->height;
		dst_fps = DIV_ROUND_CLOSEST(gc4c33->cur_mode->max_fps.denominator,
			gc4c33->cur_mode->max_fps.numerator);
		for (i = 0; i < gc4c33->cfg_num; i++) {
		...
		if (i == gc4c33->cfg_num) {
			...
			gc4c33->cur_mode = &supported_modes[cur_best_fit];
