...
struct ov16a1q {
	...
	struct regulator_bulk_data supplies[OV16A1Q_NUM_SUPPLIES];
...
static long ov16a1q_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		...
		if (hdr_cfg->hdr_mode == ov16a1q->cur_mode->hdr_mode)
			return 0;
		w = ov16a1q->cur_mode->width;
		h = ov16a1q->cur_mode->height;
		dst_fps = DIV_ROUND_CLOSEST(ov16a1q->cur_mode->max_fps.denominator,
		...
		if (i == ov16a1q->cfg_num) {
			...
			ov16a1q->cur_mode = &supported_modes[cur_best_fit];
...
static int __ov16a1q_power_on(struct ov16a1q *ov16a1q)
{
	...
	ret = regulator_bulk_enable(OV16A1Q_NUM_SUPPLIES, ov16a1q->supplies);
	if (ret < 0) {
...
static void __ov16a1q_power_off(struct ov16a1q *ov16a1q)
{
	...
	regulator_bulk_disable(OV16A1Q_NUM_SUPPLIES, ov16a1q->supplies);
}
...
{
	return i2c_add_driver(&ov16a1q_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&ov16a1q_i2c_driver);
}
