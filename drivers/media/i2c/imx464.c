...
static long imx464_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		hdr = (struct rkmodule_hdr_cfg *)arg;
		if (hdr->hdr_mode == imx464->cur_mode->hdr_mode)
			return 0;
		w = imx464->cur_mode->width;
		h = imx464->cur_mode->height;
		dst_fps = DIV_ROUND_CLOSEST(imx464->cur_mode->max_fps.denominator,
			imx464->cur_mode->max_fps.numerator);
		for (i = 0; i < imx464->cfg_num; i++) {
		...
		if (i == imx464->cfg_num) {
			...
			imx464->cur_mode = &imx464->support_modes[cur_best_fit];
