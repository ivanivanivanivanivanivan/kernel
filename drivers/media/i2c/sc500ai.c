...
static const struct sc500ai_mode *sc500ai_find_mode(struct sc500ai *sc500ai, int fps)
{
	const struct sc500ai_mode *mode = NULL;
	const struct sc500ai_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == sc500ai->cur_mode->width &&
		    mode->height == sc500ai->cur_mode->height &&
		    mode->hdr_mode == sc500ai->cur_mode->hdr_mode &&
		    mode->bus_fmt == sc500ai->cur_mode->bus_fmt) {
...
static int sc500ai_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct sc500ai *sc500ai = to_sc500ai(sd);
	const struct sc500ai_mode *mode = NULL;
	...
	mode = sc500ai_find_mode(sc500ai, fps);
	if (mode == NULL) {
...
static long sc500ai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		hdr = (struct rkmodule_hdr_cfg *)arg;
		if (hdr->hdr_mode == sc500ai->cur_mode->hdr_mode)
			return 0;
		w = sc500ai->cur_mode->width;
		h = sc500ai->cur_mode->height;
		dst_fps = DIV_ROUND_CLOSEST(sc500ai->cur_mode->max_fps.denominator,
			sc500ai->cur_mode->max_fps.numerator);
		for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
			if (w == supported_modes[i].width &&
			    h == supported_modes[i].height &&
			    supported_modes[i].hdr_mode == hdr->hdr_mode &&
			    supported_modes[i].bus_fmt == sc500ai->cur_mode->bus_fmt) {
		...
		if (i == ARRAY_SIZE(supported_modes)) {
			...
			sc500ai->cur_mode = &supported_modes[cur_best_fit];
...
static const struct v4l2_subdev_video_ops sc500ai_video_ops = {
	...
	.s_frame_interval = sc500ai_s_frame_interval,
};
