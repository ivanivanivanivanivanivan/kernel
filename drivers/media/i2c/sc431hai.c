...
#define SC431HAI_LANES			2
#define SC431HAI_BITS_PER_SAMPLE	10
#define SC431HAI_LINK_FREQ_315		315000000

#define PIXEL_RATE_WITH_315M_10BIT	(SC431HAI_LINK_FREQ_315 * 2 * \
					SC431HAI_LANES / SC431HAI_BITS_PER_SAMPLE)

#define SC431HAI_XVCLK_FREQ		24000000
...
#define	SC431HAI_EXPOSURE_MIN		1
...
#define SC431HAI_GROUP_HOLD_END		0x30 // Not used
 ...
 * binning to 1280x720
 */
static const struct regval sc431hai_linear_10_1280x720_112fps_regs[] = {
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	{0x3018, 0x3a},
 ...
 * mipi_datarate per lane 630Mbps, 2lane
 */
static const struct regval sc431hai_linear_10_2560x1440_30fps_regs[] = {
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	{0x3018, 0x3a},
...
static const struct sc431hai_mode supported_modes[] = {
	{
		...
		.reg_list = sc431hai_linear_10_2560x1440_30fps_regs,
		.hdr_mode = NO_HDR,
		.bpp = 10,
		.xvclk_freq = 24000000,
		.link_freq_idx = 0,
	...
	},
	{
		...
		.vts_def = 0x02ee,
		.bus_fmt = MEDIA_BUS_FMT_SBGGR10_1X10,
		.reg_list = sc431hai_linear_10_1280x720_112fps_regs,
		.hdr_mode = NO_HDR,
		.bpp = 10,
		.xvclk_freq = 24000000,
		.link_freq_idx = 0,
...
static const char * const sc431hai_test_pattern_menu[] = {
	"Disabled",
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		...
		*v4l2_subdev_get_try_format(sd, cfg, fmt->pad) = fmt->format;
		...
		dst_pixel_rate = (u32)link_freq_menu_items[mode->link_freq_idx] /
						 SC431HAI_BITS_PER_SAMPLE * 2 * SC431HAI_LANES;
		__v4l2_ctrl_s_ctrl_int64(sc431hai->pixel_rate,
					 dst_pixel_rate);
		__v4l2_ctrl_s_ctrl(sc431hai->link_freq,
				   dst_link_freq);
...
static const struct sc431hai_mode *sc431hai_find_mode(struct sc431hai *sc431hai, int fps)
{
	const struct sc431hai_mode *mode = NULL;
	const struct sc431hai_mode *match = NULL;
	int cur_fps = 0;
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		mode = &supported_modes[i];
		if (mode->width == sc431hai->cur_mode->width &&
		    mode->height == sc431hai->cur_mode->height &&
		    mode->hdr_mode == sc431hai->cur_mode->hdr_mode &&
		    mode->bus_fmt == sc431hai->cur_mode->bus_fmt) {
...
static int sc431hai_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
	struct sc431hai *sc431hai = to_sc431hai(sd);
	const struct sc431hai_mode *mode = NULL;
	...
	mode = sc431hai_find_mode(sc431hai, fps);
	if (mode == NULL) {
	...
	__v4l2_ctrl_modify_range(sc431hai->vblank, vblank_def,
				 SC431HAI_VTS_MAX - mode->height,
				 1, vblank_def);
	__v4l2_ctrl_s_ctrl(sc431hai->link_freq, mode->link_freq_idx);
	pixel_rate = (u32)link_freq_menu_items[mode->link_freq_idx] /
		     mode->bpp * 2 * SC431HAI_LANES;
	__v4l2_ctrl_s_ctrl_int64(sc431hai->pixel_rate, pixel_rate);
	sc431hai->cur_fps = mode->max_fps;
...
static long sc431hai_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RKMODULE_SET_HDR_CFG:
		hdr = (struct rkmodule_hdr_cfg *)arg;
		if (hdr->hdr_mode == sc431hai->cur_mode->hdr_mode)
			return 0;
		w = sc431hai->cur_mode->width;
		h = sc431hai->cur_mode->height;
		dst_fps = DIV_ROUND_CLOSEST(sc431hai->cur_mode->max_fps.denominator,
			sc431hai->cur_mode->max_fps.numerator);
		for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
			if (w == supported_modes[i].width &&
			    h == supported_modes[i].height &&
			    supported_modes[i].hdr_mode == hdr->hdr_mode &&
				supported_modes[i].bus_fmt == sc431hai->cur_mode->bus_fmt) {
		...
		if (i == ARRAY_SIZE(supported_modes)) {
			...
			sc431hai->cur_mode = &supported_modes[cur_best_fit];
			w = sc431hai->cur_mode->hts_def - sc431hai->cur_mode->width;
			h = sc431hai->cur_mode->vts_def - sc431hai->cur_mode->height;
			__v4l2_ctrl_modify_range(sc431hai->hblank, w, w, 1, w);
			__v4l2_ctrl_modify_range(sc431hai->vblank, h,
						 SC431HAI_VTS_MAX - sc431hai->cur_mode->height,
						 1, h);
			sc431hai->cur_fps = sc431hai->cur_mode->max_fps;
		}
		break;
				   ...
				   unsigned int cmd, unsigned long arg)
{
	...
	case PREISP_CMD_SET_HDRAE_EXP:
		...
		if (!ret)
			ret = sc431hai_ioctl(sd, cmd, hdrae);
		else
			ret = -EFAULT;
		kfree(hdrae);
		break;
	case RKMODULE_SET_QUICK_STREAM:
		...
		if (!ret)
			ret = sc431hai_ioctl(sd, cmd, &stream);
		else
			ret = -EFAULT;
		break;
	default:
...
static int __sc431hai_start_stream(struct sc431hai *sc431hai)
{
	...
	if (!sc431hai->is_thunderboot) {
		...
		if (ret)
			return ret;
		if (sc431hai->has_init_exp && sc431hai->cur_mode->hdr_mode != NO_HDR) {
			ret = sc431hai_ioctl(&sc431hai->subdev, PREISP_CMD_SET_HDRAE_EXP,
				&sc431hai->init_hdrae_exp);
			if (ret) {
				dev_err(&sc431hai->client->dev,
...
static int __sc431hai_stop_stream(struct sc431hai *sc431hai)
{
	...
	return sc431hai_write_reg(sc431hai->client, SC431HAI_REG_CTRL_MODE,
				 SC431HAI_REG_VALUE_08BIT, SC431HAI_MODE_SW_STANDBY);
}
...
static inline u32 sc431hai_cal_delay(u32 cycles, struct sc431hai *sc431hai)
{
	return DIV_ROUND_UP(cycles, sc431hai->cur_mode->xvclk_freq / 1000 / 1000);
}
...
static int __sc431hai_power_on(struct sc431hai *sc431hai)
{
	...
	ret = clk_set_rate(sc431hai->xvclk, sc431hai->cur_mode->xvclk_freq);
	if (ret < 0)
		dev_warn(dev, "Failed to set xvclk rate (%dHz)\n", sc431hai->cur_mode->xvclk_freq);
	if (clk_get_rate(sc431hai->xvclk) != sc431hai->cur_mode->xvclk_freq)
	...
	cam_sw_regulator_bulk_init(sc431hai->cam_sw_inf, SC431HAI_NUM_SUPPLIES, sc431hai->supplies);
	...
	delay_us = sc431hai_cal_delay(8192, sc431hai);
...
static int __maybe_unused sc431hai_resume(struct device *dev)
{
	int ret;
	...
	cam_sw_write_array(sc431hai->cam_sw_inf);

	if (__v4l2_ctrl_handler_setup(&sc431hai->ctrl_handler))
		dev_err(dev, "__v4l2_ctrl_handler_setup fail!");

	if (sc431hai->has_init_exp && sc431hai->cur_mode != NO_HDR) {	// hdr mode
		ret = sc431hai_ioctl(&sc431hai->subdev, PREISP_CMD_SET_HDRAE_EXP,
				    &sc431hai->cam_sw_inf->hdr_ae);
		if (ret) {
			dev_err(&sc431hai->client->dev, "set exp fail in hdr mode\n");
			return ret;
		}
	}
	return 0;
...
static int __maybe_unused sc431hai_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc431hai *sc431hai = to_sc431hai(sd);
	...
	cam_sw_prepare_sleep(sc431hai->cam_sw_inf);
...
static int __maybe_unused sc431hai_runtime_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct v4l2_subdev *sd = i2c_get_clientdata(client);
	struct sc431hai *sc431hai = to_sc431hai(sd);
...
static const struct v4l2_subdev_video_ops sc431hai_video_ops = {
	...
	.s_frame_interval = sc431hai_s_frame_interval,
};
...
static int sc431hai_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		...
		max = sc431hai->cur_mode->height + ctrl->val - 5;
	...
	case V4L2_CID_VBLANK:
		...
		if (sc431hai->cur_vts != sc431hai->cur_mode->vts_def)
	...
	case V4L2_CID_HFLIP:
		ret = sc431hai_read_reg(sc431hai->client, SC431HAI_FLIP_MIRROR_REG,
				       SC431HAI_REG_VALUE_08BIT, &val);
		ret |= sc431hai_write_reg(sc431hai->client, SC431HAI_FLIP_MIRROR_REG,
					 SC431HAI_REG_VALUE_08BIT,
...
static int sc431hai_initialize_controls(struct sc431hai *sc431hai)
{
	...
	if (sc431hai->link_freq)
		sc431hai->link_freq->flags |= V4L2_CTRL_FLAG_READ_ONLY;

	dst_link_freq = mode->link_freq_idx;
	dst_pixel_rate = (u32)link_freq_menu_items[mode->link_freq_idx] /
					 SC431HAI_BITS_PER_SAMPLE * 2 * SC431HAI_LANES;
	sc431hai->pixel_rate = v4l2_ctrl_new_std(handler, NULL, V4L2_CID_PIXEL_RATE,
			  0, PIXEL_RATE_WITH_315M_10BIT, 1, dst_pixel_rate);

	__v4l2_ctrl_s_ctrl(sc431hai->link_freq, dst_link_freq);
	...
	sc431hai->exposure = v4l2_ctrl_new_std(handler, &sc431hai_ctrl_ops,
					      V4L2_CID_EXPOSURE, SC431HAI_EXPOSURE_MIN,
					      exposure_max, SC431HAI_EXPOSURE_STEP,
					      mode->exp_def);
	sc431hai->anal_gain = v4l2_ctrl_new_std(handler, &sc431hai_ctrl_ops,
					       V4L2_CID_ANALOGUE_GAIN, SC431HAI_GAIN_MIN,
					       SC431HAI_GAIN_MAX, SC431HAI_GAIN_STEP,
					       SC431HAI_GAIN_DEFAULT);
	sc431hai->test_pattern = v4l2_ctrl_new_std_menu_items(handler,
							    &sc431hai_ctrl_ops,
					V4L2_CID_TEST_PATTERN,
					ARRAY_SIZE(sc431hai_test_pattern_menu) - 1,
					0, 0, sc431hai_test_pattern_menu);
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (!sc431hai->cam_sw_inf) {
		sc431hai->cam_sw_inf = cam_sw_init();
		cam_sw_clk_init(sc431hai->cam_sw_inf, sc431hai->xvclk,
			sc431hai->cur_mode->xvclk_freq);
		cam_sw_reset_pin_init(sc431hai->cam_sw_inf, sc431hai->reset_gpio, 0);
		cam_sw_pwdn_pin_init(sc431hai->cam_sw_inf, sc431hai->pwdn_gpio, 1);
	}
...
static int sc431hai_remove(struct i2c_client *client)
{
	...
	cam_sw_deinit(sc431hai->cam_sw_inf);
